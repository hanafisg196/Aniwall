...
class FavoriteDataResource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Wallpapers> {
        return try {
            ...
            val response = repository.favoriteWallpapers(token,userId,page, Constants.ITEM_PAGE)
//            Log.d("Favorites", "Data: ${response.data}")
            LoadResult.Page(
                data = response.data,
...
//            Log.e("Favorites", "Error saat memuat data favorites: ${e.message}", e)
            LoadResult.Error(e)
        }
    }
